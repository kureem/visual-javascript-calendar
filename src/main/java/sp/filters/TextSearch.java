...
//	private boolean remoteSearch = false;
	...
	private String objectType = "ContentVersion";
	...
	public TextSearch(String name) {
		...
		wrap.addChild(bd);
		wrap.addChild(list);
		// list.setStyle("height", "150px").setStyle("overflow-y",
		// "auto").setStyle("overflow-x", "hidden");

		list.addClass("txt-search-options");
		list.setStyle("height", "150px").setStyle("overflow", "hidden auto").setStyle("border", "solid 1px silver")
				.setStyle("margin", "0.25rem").setStyle("padding", "0.25rem").setStyle("border-radius", "5px");
	...
	public void setDateOptions() {
		...
		addOpt("0-days", "Today and before", opts);
		addOpt("1-days", "Before last 1 day", opts);
		addOpt("2-days", "Before last 2 days", opts);
		addOpt("3-days", "Before last 3 days", opts);
		addOpt("7-days", "Before last week", opts);
		addOpt("14-days", "Before last fortnight", opts);
		addOpt("31-days", "Before last month", opts);
		addOpt("90-days", "Before last 3 months", opts);
		setData(opts);
	}
	...
	/*
	 * private boolean requireRefresh() { String type =
	 * (String)column_.$get("type"); String sfType = (String)column_.$get("sfType");
	 * if(type == "Boolean") { return false; }
	 * 
	 * if(sfType == "PICKLIST" || sfType == "PICKLIST" || sfType == "MULTIPICKLIST"
	 * || sfType == "DATE" || sfType == "DATETIME") { return false; }
	 * 
	 * return true; }
	 */

	/*
	 * private boolean isPicklist() { String sfType =
	 * (String)column_.$get("sfType");
	 * 
	 * if(sfType == "PICKLIST" || sfType == "PICKLIST" || sfType == "MULTIPICKLIST")
	 * { return true; }
	 * 
	...
	/*
	 * @SuppressWarnings("unchecked") public void doRemoteSearch(boolean force) {
	 * 
	 * String type = (String)column_.$get("type"); String sfType =
	 * (String)column_.$get("sfType"); if(sfType == "DATE" || sfType == "DATETIME")
	 * { setDateOptions(); return; }
	 * 
	 * if(type == "Boolean") { if(column_.hasOwnProperty("includes")) {
	 * Array<String> incl = (Array<String>)column_.$get("includes");
	 ...
	 * 
	 * String fieldName = getName().replace("textSearch_", ""); String txt =
	 * input.getValue(); //boolean picklist = isPicklist(); if(txt != null &&
	 * txt.trim().length() > 0 || picklist || force) { txt = txt.replace("*", "%");
	 * int page = 0; if(force) { txt = "%"; }
	 * 
	 * String va= txt; //if(timeoutHandle != -1) { // clearTimeout(timeoutHandle);
	 * //timeoutHandle = -1; //} GridHeader hg = getAncestorWithClass("GridHeader");
	 * String filterId = hg.getFilterId();
	 * 
	 * timeoutHandle = setTimeout(function(()->{ this.searching = true;
	 * //bd.setStyle("visiblity", "visible").setStyle("display",
	 * null).setStyle("opacity", "1"); //input.setReadOnly(true); //bd.render();
	 * list.setStyle("opacity", "0.4").setStyle("background", "silver").render();
	 * Util.getService().getFieldValues(fieldName,va,page,objectType,
	 * filterId).then((result)->{ searching = false; if(!requireRefresh()) { cached
	 * = true; } //bd.setStyle("visiblity", null).setStyle("display",
	 * "none").setStyle("opacity", null); //input.setValue(txt);
	 * list.setStyle("opacity", null).setStyle("background", null);
	 * setRendered(false); setData(result); if(column_.hasOwnProperty("includes")) {
	 * Array<String> incl = (Array<String>)column_.$get("includes");
	 * setSelectedOptions(incl); } render(); HTMLInputElement el =
	 * (HTMLInputElement)input.getElement(); el.focus();
	 * el.setSelectionRange(el.value.length(), el.value.length());
	 * //input.getElement().focus(); }).Catch((e)->{ searching = false; });
	 * 
	...
	/*
	 * public void setTree(Tree tree) { list.clearChildren(); JSContainer selall =
	 * new JSContainer("li"); selall.addChild(tree); list.addChild(selall); }
	 */
	...
	public void setData(Array<Object> data) {
		...
		if (data != null) {
			...
			}, "change")).addChild(new JSContainer("span").setStyle("font-weight", "bold").addClass("slds-truncate")
					.setHtml("Select All"));
			...
			for (Object line : data) {
				...
				/*
				 * Object selItem = getInSelected(line, selected); if (selItem != null) {
				 * item.setSelected(true); used.push(line); }
				 */
				list.addChild(item);
			}
			// more seleted than searched
			/*
			 * if (used.length < selected.length) { for (Object sel : selected) { Object ins
			 * = getInSelected(sel, used); if (ins == null) { ListItem item =
			 * getListItem(sel); item.setSelected(true); list.addChildAt(0, item); } } }
			 */
		}
	}
