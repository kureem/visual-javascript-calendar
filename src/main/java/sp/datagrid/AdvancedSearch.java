...
public class AdvancedSearch extends Modal{
	...
	public AdvancedSearch(String name) {
		...
		//values.setStrict(false);
		//values2.setStrict(false);
		//values.setVariant(FormElementVariant.LABEL_HIDDEN);
		//values2.setVariant(FormElementVariant.LABEL_HIDDEN);
		

		/*values.getCombo().getInput().addEventListener(new EventListener() {
			
			@Override
			public void performAction(Renderable source, Event evt) {
				String txt = values.getCombo().getInput().getValue();
				if(column != null) {
					if(txt.length() % 3 == 0) {
						String fieldName = (String)column.$get("fieldName");
						Util.getService().getFieldValues(fieldName, txt, 0, objectType).then(result->{
							values.setOptions(result);
							values.getCombo().setExpand(true);
							values.getCombo().getDropdown().setRendered(false);
							values.getCombo().render();
						});
					}
				}
			}
		...
		values2.getCombo().getInput().addEventListener(new EventListener() {
			
			@Override
			public void performAction(Renderable source, Event evt) {
				String txt = values2.getCombo().getInput().getValue();
				if(column != null) {
					if(txt.length() % 3 == 0) {
						String fieldName = (String)column.$get("fieldName");
						Util.getService().getFieldValues(fieldName, txt, 0, objectType).then(result->{
							values2.setOptions(result);
							values2.getCombo().setExpand(true);
							values2.getCombo().getDropdown().setRendered(false);
							values2.getCombo().render();
						});
					}
				}
			}
