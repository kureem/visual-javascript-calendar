...
public class TableTitle extends JSContainer{
	...
	public TableTitle(String name, boolean dry) {
		super(name, "div");
		...
		addChild(iconctn);
		icon.setAttribute("src", "https://ceva--uat.sandbox.lightning.force.com/img/icon/t4v35/standard/case_120.png");
		...
		if(!dry) {
			...
			switcher.addChild(pin);
			pin.setTitle("Pin this list");
			pin.addEventListener(new EventListener() {
				
				@Override
				public void performAction(Renderable source, Event evt) {
					if(pin.getIconName() == "utility:pin") {
						HeavyGrid gr = source.getAncestorWithClass("HeavyGrid");
						gr.pinCurrent();
						pin.setIconName("utility:pinned");
					}else {
						alert("Pin another list");
					}
				}
			}, "click");
			pin.setVariant(ButtonIconVariant.BARE);
	...
	public void doOpenListFilters(Renderable source) {
