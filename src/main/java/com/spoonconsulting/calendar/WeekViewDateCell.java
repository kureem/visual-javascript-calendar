...
public class WeekViewDateCell extends Box  {
	...
	public WeekViewDateCell(Date date, double hr,int days, int row, int col ) {
		super(Util.formatDate(date, "dd"), 1, days);
		this.date = date;
		this.hour = hr;
		addClass("spn-body-right-body-cell");
		cellHr = new WeekViewCell(hr, 0, date,row,col);
		cellHalfHr = new WeekViewCell(hr, 30, date, row++,col);
